...
def CA_RUN(cluster_num=30, iterations=50):
    # Read data (in_centers.csv, in_data.csv only things that are required)
    
    # A1 Synthetic Dataset Example
    #"""
    in_centers = pd.read_csv('random-centers.csv', header=None)
        
    in_data = pd.read_csv('data/A1-Dataset/Final Dataset - thirdtrial.csv', header=None)
    ...
    CA_cluster_num, CA_center_list, CA_classification_list, MMembership = CA(in_data, in_centers, max_iterations=iterations,
    ...
    return(CA_cluster_num, CA_center_list, CA_classification_list, MMembership)
